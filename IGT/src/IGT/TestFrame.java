...
public class TestFrame extends JFrame {
	...
	private int numberOfPracticeBlock = 1;
	private int numberOfMainBlock = 4;
	private int numberOfTrialOnEachBlock = 20;
	
	private int currentTrialOnBlock;
	
	//Flow Thread
	private Thread practiceStartThread = new Thread(new Runnable() {
		...
		public void run() {	
			...
			SwingUtilities.invokeLater(new Runnable() {
				...
				@Override
				public void run() {
					cardLayout.show(contentPanel, "PracticeStart");
				}
			});
	...
	private Thread practiceEndThread = new Thread(new Runnable() {
		...
		public void run() {
			...
			SwingUtilities.invokeLater(new Runnable() {
				...
				@Override
				public void run() {
					cardLayout.show(contentPanel, "PracticeEnd");
				}
			});	
	...
	private Thread mainStartThread = new Thread(new Runnable() {
		...
		public void run() {
			...
			SwingUtilities.invokeLater(new Runnable() {
				...
				@Override
				public void run() {
					cardLayout.show(contentPanel, "MainStart");
				}
			});
			try {
				Thread.sleep(2000);
	...
	private Thread mainEndThread = new Thread(new Runnable() {
		...
		public void run() {
			...
			SwingUtilities.invokeLater(new Runnable() {
				...
				@Override
				public void run() {
					cardLayout.show(contentPanel, "MainEnd");
				}
			});
	...
	public TestFrame(String subjectNumber, String subjectName, int settingValues[]) throws Exception {
		...
		contentPanel.add(getMessagePanelWithButton("테스트를 시작하겠습니다", "Start", new ActionListener() {
			
			@Override
			public void actionPerformed(ActionEvent e) {
				practiceStartThread.start();
			}
		}), "Start");
		contentPanel.add(getMessagePanelWithButton("모든 테스트가 종료되었습니다.", "결과를 저장하고 종료", new ActionListener() {
						...
						try {
							...
							if (resultsDir.exists()) {
								out = new BufferedWriter(new OutputStreamWriter(
										new FileOutputStream("Results/" + subjectNumber_ + ".csv"), "UTF8"));
							}
							else {
								out = new BufferedWriter(new OutputStreamWriter(
										new FileOutputStream("" + subjectNumber_ + ".csv"), "UTF8"));
							}
		...
		if (!isCardAction.getAndSet(true)) {
			...
			new SwingWorker<Void, Void>() {
				...
				protected Void doInBackground() throws Exception {
					...
					SwingUtilities.invokeLater(new Runnable() {
						...
						public void run() {
							...
							for (int i = 0; i < cards.length; i++) {
								cards[i].getButton().setEnabled(false);
							}
						}
					});
					int currnetCardIndex = currentCard.getIndex();
					int[] money = decks.getCard(currnetCardIndex);
					...
					//결과 저장
					blockCount[currnetCardIndex]++;
					switch (currnetCardIndex) {
					case 0:
						blockResult += "A,";
						break;
					case 1:
	...
	private JPanel getMainTestPanel() {
		...
		for (int i = 0; i < cards.length; i++) {
			...
			currentCard.getButton().addActionListener(new ActionListener() {
				@Override
				public void actionPerformed(ActionEvent e) {
